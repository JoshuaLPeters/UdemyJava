...
//    returns a char array containing every nth char. When sourceArray.length < n, returns sourceArray
    public char[] everyNthChar(char[] sourceArray, int n){
        ...
        for(int i = n-1; i< sourceArray.length; i+= i){
            result[index++] = sourceArray[i];
        }
        return result;

    }
    //removes pairs of the same character that are next to each other by removing on the e occurrence of the character
    // ABCDEEF -> ABCDEF
